...
public class TaskManager {
    ...
    public Subtask getSubtaskById(int id) {                 //Получение поздадачи по id
    ...
    public List<Subtask> getSubtasksOfEpic(int epicId) {   //Получение списка подзадая из эпика по id
    ...
    private void updateEpicStatus(Epic epic) {              //Автоматическое обновление статуса эпика
        List<Integer> subtaskIds = epic.getSubtaskIds();    //Получаем список ID всех подзадач с этим эпиком.
        if (subtaskIds.isEmpty()) {
            epic.setStatus(TaskStatus.NEW); //Если у эпика вообще нет подзадач, то статус по умолчанию NEW.
            return; //Выход из метода, дальше не считаем.
        }

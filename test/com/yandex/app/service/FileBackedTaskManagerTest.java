...
class FileBackedTaskManagerTest {
     ...
     */
    @Test
    void shouldNotThrowWhenSavingOrLoadingCorruptedFile() {
        // Сохраняем корректный менеджер
        Task task = new Task("Тест", "Описание", TaskStatus.NEW);
        manager.addTask(task);

        // Портим файл вручную
        try {
            java.nio.file.Files.writeString(tempFile.toPath(), "broken,data,file,structure");
        } catch (IOException e) {
